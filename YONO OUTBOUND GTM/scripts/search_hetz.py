...
def search_hetz_threads():
    ...
    # Search for threads with Hetz
    query = 'hetzventures.com OR "Itamar" OR "Pavel"'
    results = service.users().threads().list(userId='me', q=query).execute()
    threads = results.get('threads', [])

    if not threads:
        print("No threads found for Hetz.")
        return

    for t in threads[:5]: # Check last 5 threads
