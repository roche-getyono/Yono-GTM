...
def search_hetz_3_months():
    ...
    # Search for "3 months" in messages related to Hetz
    query = 'hetz.vc "3 months"'
    results = service.users().messages().list(userId='me', q=query).execute()
    messages = results.get('messages', [])

    if not messages:
        # Try a broader search if no direct match
        print("No direct '3 months' + hetz match. Trying broader search for '3 months'...")
        query = '"3 months"'
        results = service.users().messages().list(userId='me', q=query).execute()
        messages = results.get('messages', [])
